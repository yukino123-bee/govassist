...
if ($method === 'GET') {
    try {
        $stmt = $pdo->query("SELECT id, service_title, date, is_eligible, reference_number FROM assessments ORDER BY date DESC");
    ...
    if (isset($input['serviceTitle']) && isset($input['isEligible']) && isset($input['date'])) {
        try {
            $stmt = $pdo->prepare("INSERT INTO assessments (service_title, is_eligible, date, reference_number) VALUES (?, ?, ?, ?)");
